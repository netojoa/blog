...
namespace SC81.Sandbox.TelligentSDK.SSO
{
    ...
    public class AesHmac : IDisposable
    {
        ...
        public AesHmac(string stringEncryptionKey, string hmacKeyString) : this(null, null, stringEncryptionKey, hmacKeyString)
        {
        }

        public AesHmac(byte[] encryptionKey, byte[] hmacKey, string stringEncryptionKey, string hmacKeyString)
        {

            if (encryptionKey == null)
                encryptionKey = Convert.FromBase64String(stringEncryptionKey);
                                         ...
                                         string cookieKeyNameForDisplayName, string displayName)
        {
            ...
            return SecureCookieForSso(ssoData);
        }

        public string SecureCookieForSso(string ssoData)
        {
